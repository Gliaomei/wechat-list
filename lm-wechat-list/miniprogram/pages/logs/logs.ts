...
Page({
  ...
  // 加载图片
  loadImage: function(e: any):void {
    let index: number = e.currentTarget.dataset.index; //图片所在索引
    let imgW: number = e.detail.width;
    let imgH: number = e.detail.height; //图片实际宽度和高度
    let imgWidth: number = this.data.imgWidth; //图片宽度
    let imgScaleH:number = imgWidth / imgW * imgH; //计算图片应该显示的高度

    let dataList = this.data.dataList;
    var margin:number = this.data.imgMargin;  //图片间距
    //第一列的累积top，和第二列的累积top
    var firtColH:number = this.data.topArr[0], secondColH = this.data.topArr[1];
    let obj:any = dataList[index];
  ...
  loadMoreImages: function ():void {
    ...
    for (let i = 0; i < 22; i++) {
      let index = parseInt(Math.random() * 100 as any) % imgs.length;
      var obj = {
        src: imgs[index],
        height: 0,
        top: 0,
    ...
    var dataList = this.data.dataList.concat(tmpArr as any);
